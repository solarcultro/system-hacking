...
```assembly
     ...
     0x0804887f <+340>:   call   0x80484e0 <__stack_chk_fail@plt>
```
이제 카나리가 저장된 곳의 위치는 파악했다. 이제 카나리 값을 알아내야한다. 카나리 값을 알아내려면 카나리 값이 있는 곳의 위치까지 overflow를 일으켜 카나리 값을 읽을 수 있게해주는 스택 버퍼를 찾아야한다. 그러기 위해서는 스택 프레임의 구조를 먼저 파악해야한다. 우선 카나리 값이 저장된 곳과 가까운 스택 버퍼의 위치부터 찾자. disassemble한 코드를 살펴보며 찾아보자. 
```assembly
   ...
   0x0804885f <+308>:   call   0x80484a0 <read@plt>
```
위 코드는 read(0, name, name_len);를 disassemble한 code이다. 이를 통해 name이 ebp-0x48에 위치함을 알 수 있다. name_len은 ebp-0x90에 위치함을 알 수 있다.
...
```assembly
   ...
   0x080487de <+179>:   call   0x80484a0 <read@plt>
```
위 코드는 read(0, box, sizeof(box));를 disassemble한 code이다. 이를 통해 box가 ebp-0x88에 위치함을 알 수 있다.
...
이제 이를 이용하여 카나리값을 읽는 방법을 생각해보자. main 함수의 switch문을 살펴보자. switch의 인자로 F와 E를 주게되면 box 또는 name에 overflow를 일으켜서 카나리 값을 읽어낼 수 있을 것이다. 하지만 E를 인자로 주게된다면 프로그램이 종료되므로 F를 인자로 주어 카나리 값을 읽어내야한다. 
이제 pwndbg로 get_shell 함수의 주소를 알아내자.
...
이제 지금까지 얻은 정보를 이용하여 Exploit script를 작성해보자.
